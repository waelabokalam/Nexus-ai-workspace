...
function FeatureDetail({ feature }: { feature: Feature }) {
  return <div>{feature.status ? <div className="flex items-center justify-between gap-4"><ProductStatus status={feature.status} /><span className="nexus-subtle text-[10px] uppercase tracking-[0.12em]">Product status</span></div> : null}<h3 className={`nexus-heading font-heading text-2xl font-medium tracking-[-0.04em] ${feature.status ? "mt-5" : ""}`}>{feature.title}</h3><dl className="mt-8 grid gap-6 text-sm leading-6"><div><dt className="nexus-subtle text-xs font-medium uppercase tracking-[0.14em]">Business problem</dt><dd className="nexus-copy mt-2">{feature.problem}</dd></div><div><dt className="nexus-subtle text-xs font-medium uppercase tracking-[0.14em]">How Nexus works</dt><dd className="nexus-copy mt-2">{feature.works}</dd></div><div><dt className="nexus-subtle text-xs font-medium uppercase tracking-[0.14em]">Business outcome</dt><dd className="nexus-heading mt-2">{feature.outcome}</dd></div></dl></div>;
}
