...
import { homeEn, type HowItWorksCopy } from "@/lib/i18n/home";

const routeIcons = [Check, ShieldCheck, UserRound] as const;

export default function HowNexusWorks({ t = homeEn.howItWorks }: { t?: HowItWorksCopy }) {
  const decisionRoutes = t.routes.map((route, index) => ({ ...route, icon: routeIcons[index] }));
  return (
        ...
        <SectionReveal className="mt-12 overflow-hidden rounded-[var(--nexus-radius-surface)] border border-[var(--nexus-border)] bg-[var(--nexus-surface-soft)] p-5 sm:p-7">
          <div className="grid gap-5 lg:grid-cols-[1fr_auto_0.8fr_auto_1fr] lg:items-stretch">
            <div className="nexus-surface rounded-[var(--nexus-radius-control)] p-5">
              <h3 className="nexus-heading text-sm font-medium">{t.operationTitle}</h3>
              <div className="mt-5 flex flex-wrap gap-2">
                {t.inputs.map((item) => <span className="nexus-control rounded-lg px-3 py-2 text-xs nexus-copy" key={item}>{item}</span>)}
              </div>
            </div>

            <div aria-hidden="true" className="hidden items-center lg:flex"><ArrowRight className="nexus-subtle size-5 rtl:-scale-x-100" strokeWidth={1.5} /></div>

            <div className="rounded-[var(--nexus-radius-control)] border border-[var(--nexus-border-strong)] bg-[var(--nexus-surface-raised)] p-5">
              <div className="flex items-center gap-3"><TqMonogram size={28} /><h3 className="nexus-heading text-sm font-semibold">{t.systemName}</h3></div>
              <div className="mt-5 flex flex-wrap gap-2">
                {t.actions.map((item) => <span className="nexus-heading text-xs" key={item}>{item}</span>)}
              </div>
            </div>
