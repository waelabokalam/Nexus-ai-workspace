    ...
    <section className="px-5 py-20 sm:px-8 sm:py-28" id="restaurant-flagship">
      <div className="mx-auto max-w-7xl">
        <SectionReveal className="grid gap-10 lg:grid-cols-[0.72fr_1.28fr] lg:items-end">
          <div>
            <span className="nexus-status inline-flex rounded-full px-3 py-1 text-xs font-medium">Restaurant V1 · Pilot ready</span>
            <h2 className="nexus-heading mt-5 font-heading text-4xl font-semibold leading-[1.02] tracking-[-0.055em] sm:text-5xl">Restaurant operations, filtered down to what needs management.</h2>
            <p className="nexus-copy mt-5 max-w-xl text-base leading-7">The Manager Command Center brings operational exceptions, approvals, reputation signals and supplier costs into one review surface.</p>
          </div>
        ...
        <SectionReveal className="mt-8 grid gap-6 border-t border-[var(--nexus-border)] pt-8 lg:grid-cols-[1fr_auto] lg:items-end">
          ...
          <Link className="nexus-button-primary nexus-focus inline-flex min-h-11 items-center justify-center rounded-[var(--nexus-radius-control)] px-5 text-sm font-medium" href="/restaurants">Explore Nexus Restaurant <ArrowRight aria-hidden="true" className="ml-2 size-4" strokeWidth={1.6} /></Link>
        </SectionReveal>
      </div>
    </section>
  );
